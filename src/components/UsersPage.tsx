  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h2 className="brand-title mt-2 text-2xl font-bold text-temple-brown sm:text-3xl">Members &amp; Access</h2>
      ...
      {isAdmin && (
        <section className="form-panel">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-temple-brown">Invite a Team Member</h3>
              <p className="text-xs text-muted">They'll receive an email with a link to set up their account</p>
            </div>
          </div>

          <form onSubmit={handleInvite} className="flex flex-col gap-3 sm:flex-row sm:items-end">
            <label className="flex-1 text-sm font-semibold text-temple-brown">
              Email address
              <input
                type="email"
                value={inviteEmail}
            ...
            <button type="submit" disabled={inviteBusy} className="btn-primary h-[42px] rounded-full">
              <MailPlus className="h-4 w-4" />
              {inviteBusy ? 'Sending...' : 'Send Invite'}
            </button>
          </form>

          {inviteError && <p className="mt-3 text-sm text-temple-outflow">{inviteError}</p>}
            ...
            <div className="mt-5 border-t border-white/50 pt-4">
              ...
              <div className="space-y-2">
                {invites.map((inv) => (
                  <div
                    key={inv.id}
                    className="flex items-center gap-3 rounded-xl border border-white/60 bg-white/45 px-4 py-2.5"
                  >
                    <Mail className="h-4 w-4 shrink-0 text-brand-saffron-600" />
                    <span className="flex-1 truncate text-sm font-medium text-temple-brown">{inv.email}</span>
                    <span className="rounded-full bg-brand-gold-400/20 px-2.5 py-0.5 text-xs font-semibold text-brand-gold-600 ring-1 ring-brand-gold-400/30">
                      Pending
                    </span>
                    <button
                      type="button"
      ...
      <section className="card overflow-hidden">
        <div className="border-b border-white/50 bg-white/30 px-5 py-4">
          <h3 className="font-bold text-temple-brown">All Members</h3>
          ...
          <div className="divide-y divide-white/50">
            {profiles.map((member) => {
              ...
              return (
                <div
                  key={member.id}
                  className="flex items-center gap-4 px-5 py-4 transition hover:bg-white/35"
                >
                  <div className="avatar-badge shrink-0">{getInitials(member.display_name)}</div>
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-semibold text-temple-brown">
                      {member.display_name}
                  ...
                  <span
                    className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold ${
                      isMemberAdmin
                        ? 'bg-brand-gold-400/20 text-brand-gold-600 ring-1 ring-brand-gold-400/30'
                        : 'bg-white/50 text-temple-muted ring-1 ring-white/60'
                    }`}
                  >
                  ...
                  {isAdmin && !isCurrentUser && (
                    <button
                      ...
                      className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition disabled:opacity-50 ${
                        isMemberAdmin
                          ? 'border-white/60 text-temple-muted hover:bg-white/50'
      ...
      {!isAdmin && (
        <div className="card flex items-start gap-3 p-5">
          ...
          <div>
            <p className="text-sm font-semibold text-temple-brown">Read-only access</p>
            <p className="mt-1 text-sm text-temple-muted">
